      ...
      <div className={styles.hitRates}>
        <Title text={t("components.hitRates.title")} />
        {
            ...
            <div className={styles.columns}>
                ...
                {[...Array(10)].map((_, i) => {
                    return <div className={styles.hitRate}><b>{i+1}</b>: {getHitRatesForSector(filteredMatches[0].turns, i+1).toFixed(2)}%</div>;
                })}
                </div>
                <div className={styles.column}>
                    {[...Array(10)].map((_, i) => {
                        return <div className={styles.hitRate}><b>{i+11}</b>: {getHitRatesForSector(filteredMatches[0].turns, i+11).toFixed(2)}%</div>;
                    })}
                    <div className={styles.hitRate}>
                        <b>{finish_on}</b>: {getHitRatesForSector(filteredMatches[0].turns, finish_on).toFixed(2)}%
                    </div>
                </div>
            </div>
        }
